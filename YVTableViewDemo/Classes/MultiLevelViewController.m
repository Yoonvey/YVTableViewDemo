...
#import "DraggingSortViewController.h"
#import "GroupedListViewController.h"
#import "LoadingListViewController.h"

static NSString * const cellId = @"UITableViewCell";

@interface MultiLevelViewController () <UITableViewDelegate, UITableViewDataSource, ChildScrollDelegate>

@property (nonatomic, strong) UITableView *listView;
@property (nonatomic, strong) NinaPagerView *ninaPagerView;

@property (nonatomic, strong) DeleteListViewController *deleteControl;
@property (nonatomic, strong) DraggingSortViewController *dragSortControl;
@property (nonatomic, strong) GroupedListViewController *groupedControl;
@property (nonatomic, strong) LoadingListViewController *loadingControl;
...
#pragma makr - <懒加载对象>
- (DeleteListViewController *)deleteControl
{
    if (!_deleteControl)
    {
        _deleteControl = [[DeleteListViewController alloc] initWithFrame:CGRectMake(0, 0, ScreenWidth, ScreenHeight - SafeAreaTopHeight - 40)];
        _deleteControl.scrollDelegate = self;
    }
    return _deleteControl;
}

- (DraggingSortViewController *)dragSortControl
{
    if (!_dragSortControl)
    {
        _dragSortControl = [[DraggingSortViewController alloc] initWithFrame:CGRectMake(0, 0, ScreenWidth, ScreenHeight - SafeAreaTopHeight - 40)];
        _dragSortControl.scrollDelegate = self;
    }
    return _dragSortControl;
}

- (GroupedListViewController *)groupedControl
{
    if (!_groupedControl)
    {
        _groupedControl = [[GroupedListViewController alloc] initWithFrame:CGRectMake(0, 0, ScreenWidth, ScreenHeight - SafeAreaTopHeight - 40)];
        _groupedControl.scrollDelegate = self;
    }
    return _groupedControl;
}

- (LoadingListViewController *)loadingControl
{
    if (!_loadingControl)
    {
        _loadingControl = [[LoadingListViewController alloc] initWithFrame:CGRectMake(0, 0, ScreenWidth, ScreenHeight - SafeAreaTopHeight - 40)];
...
- (NinaPagerView *)ninaPagerView
{
    if (!_ninaPagerView)
    {
        NSArray *titles = [NSArray arrayWithObjects:@"拖动排序", @"分组列表", @"加载样式", @"单行删除", nil];
...
- (void)setupCommon
{
    ...
    self.listDatas = [NSMutableArray arrayWithObjects:@"测试行一", @"测试行二", @"测试行三", @"测试行四", nil];
}
...
- (void)setupListView
{
    ...
    __weak MultiLevelViewController *weakSelf = self;
...
{
    return section == 0 ? self.listDatas.count: 0;
}
